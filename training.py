...
"""Calculate 10 distance distribution for the 10 base pairs"""
...
def distance_nucleotide_pairs(n1, n2, ln):
  NN=[]
  for i in range(ln):
    if (nucl_pairs[i][0]==n1 and nucl_pairs[i][1]==n2) or (nucl_pairs[i][0]==n1 and nucl_pairs[i][1]==n2):
        if distance[i]<=20:
...
for i in range(20):
    ...
    for elem in distance_20:
        if elem<=(i+1) and elem>=i:
            count+=1
    freq_ref.append(count/len(distance_20))
len(freq_ref)
